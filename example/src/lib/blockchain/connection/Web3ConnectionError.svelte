<script lang="ts">
	import { type connection as Connection, type network as Network, account } from './';
	...
	<Banner>
		<p class="main-message">
			{$builtin.vendor === 'Metamask'
				? 'Block cache detected, Metamask  😭'
				: 'Block cache detected'}
		</p>
...
{:else if $network.hasEncounteredBlocksCacheIssue}
	<Banner>
		...
		<button tabindex="0" on:click={() => network.acknowledgeBlockCacheIssue()}> Clear </button>
	</Banner>
